...
export const ModelSettings = ({ isDarkMode = false }: ModelSettingsProps) => {
  ...
  const renderModelSelect = (agentName: AgentNameEnum) => (
    <div
      className={`rounded-lg border ${isDarkMode ? 'border-gray-700 bg-transparent' : 'border-gray-200 bg-transparent'} p-4`}>
  ...
  return (
    <section className="space-y-6">
      ...
      <div
        className={`rounded-lg border ${isDarkMode ? 'border-slate-700 bg-transparent' : 'border-blue-100 bg-transparent'} p-6 text-left shadow-sm`}>
            ...
            getSortedProviders().map(([providerId, providerConfig]) => {
              ...
              return (
                <div
                  ...
                  className={`space-y-4 ${modifiedProviders.has(providerId) && !providersFromStorage.has(providerId) ? `rounded-lg border p-4 ${isDarkMode ? 'border-blue-700 bg-transparent' : 'border-blue-200 bg-transparent'}` : ''}`}>
            ...
            {isProviderSelectorOpen && (
              <div
                className={`absolute z-10 mt-2 w-full overflow-hidden rounded-md border ${
                  isDarkMode
                    ? 'border-blue-600 bg-transparent shadow-lg shadow-slate-900/50'
                    : 'border-blue-200 bg-transparent shadow-xl shadow-blue-100/50'
                }`}>
                <div className="py-1">
