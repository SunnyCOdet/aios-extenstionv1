...
export default memo(function MessageList({ messages, isDarkMode = false, currentMode = 'agent' }: MessageListProps) {
  return (
...
function MessageBlock({ message, isSameActor, isDarkMode = false, currentMode = 'agent' }: MessageBlockProps) {
  if (!message.actor) {
  ...
  const isPlanning = message.content === 'Planning...';
        ...
        <div className="space-y-0.5">
          <div className={`whitespace-pre-wrap break-words text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            {isProgress ? (
              ...
              <div className="flex items-center gap-2">
                <span className="text-cyan-400">Planning</span>
                <div className="flex gap-1">
                  <div
          ...
          {!isProgress && !isThinking && !isPlanning && (
