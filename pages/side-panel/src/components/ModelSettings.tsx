...
// Helper function to get API key URL based on provider type
function getApiKeyUrl(providerType: string): string | null {
  const urls: Record<string, string> = {
    [ProviderTypeEnum.OpenAI]: 'https://platform.openai.com/account/api-keys',
    [ProviderTypeEnum.Anthropic]: 'https://console.anthropic.com/account/keys',
    [ProviderTypeEnum.DeepSeek]: 'https://platform.deepseek.com/api-keys',
    [ProviderTypeEnum.Gemini]: 'https://aistudio.google.com/app/apikey',
    [ProviderTypeEnum.Grok]: 'https://x.ai/api',
    [ProviderTypeEnum.AzureOpenAI]: 'https://portal.azure.com/',
    [ProviderTypeEnum.OpenRouter]: 'https://openrouter.ai/settings/keys',
    [ProviderTypeEnum.Groq]: 'https://console.groq.com/keys',
    [ProviderTypeEnum.Cerebras]: 'https://inference.cerebras.ai/account/api-keys',
  };
...
export const ModelSettings = ({ isDarkMode = false }: ModelSettingsProps) => {
  ...
  const renderModelSelect = (agentName: AgentNameEnum) => (
    <div
      className={`rounded-lg border ${isDarkMode ? 'border-gray-700 bg-transparent' : 'border-gray-200 bg-transparent'} p-3`}>
      <h3 className={`mb-1 text-base font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
        {agentName.charAt(0).toUpperCase() + agentName.slice(1)}
      </h3>
      <p className={`mb-3 text-xs font-normal ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center">
          <label
            ...
            className={`w-16 text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            {t('options_models_labels_model')}
          </label>
          <select
            id={`${agentName}-model`}
            className={`flex-1 rounded-md border text-sm ${isDarkMode ? 'border-slate-600 bg-transparent text-gray-200' : 'border-gray-300 bg-transparent text-gray-700'} px-3 py-2`}
            ...
            <div className="flex flex-1 items-center space-x-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <input
                  ...
                  className={`w-20 rounded-md border ${isDarkMode ? 'border-slate-600 bg-transparent text-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-800' : 'border-gray-300 bg-transparent text-gray-700 focus:border-blue-400 focus:ring-2 focus:ring-blue-200'} px-2 py-1 text-sm`}
  ...
  return (
    <section className="space-y-4 pb-4">
      ...
      <div
        className={`rounded-lg border ${isDarkMode ? 'border-slate-700 bg-transparent' : 'border-blue-100 bg-transparent'} p-4 text-left shadow-sm`}>
        <h2 className={`mb-3 text-lg font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
        ...
        <div className="space-y-4">
            ...
            getSortedProviders().map(([providerId, providerConfig]) => {
              ...
              return (
                <div
                  ...
                  className={`space-y-3 ${modifiedProviders.has(providerId) && !providersFromStorage.has(providerId) ? `rounded-lg border p-3 ${isDarkMode ? 'border-blue-700 bg-transparent' : 'border-blue-200 bg-transparent'}` : ''}`}>
                  <div className="flex items-center justify-between">
                    <h3 className={`text-base font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      {providerConfig.name || providerId}
                    </h3>
                    <div className="flex space-x-2">
                      {/* Get API Key button - moved here before Delete button */}
                      {getApiKeyUrl(providerConfig.type) && (
                        <a
                          href={getApiKeyUrl(providerConfig.type)!}
                          target="_blank"
                          rel="noopener noreferrer"
                          className={`flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-md transition-all duration-200 whitespace-nowrap ${
                            isDarkMode
                      ...
                      {modifiedProviders.has(providerId) && !providersFromStorage.has(providerId) && (
                        <Button variant="secondary" size="sm" onClick={() => handleCancelProvider(providerId)}>
                  ...
                  <div className="space-y-2">
                          ...
                          <input
                            ...
                            className={`flex-1 rounded-md border p-2 text-sm ${
                              nameErrors[providerId]
                                ? isDarkMode
                                  ? 'border-red-700 bg-transparent text-gray-200 focus:border-red-600 focus:ring-2 focus:ring-red-900'
                                  : 'border-red-300 bg-transparent focus:border-red-400 focus:ring-2 focus:ring-red-200'
                                : isDarkMode
                                  ? 'border-blue-700 bg-transparent text-gray-200 focus:border-blue-600 focus:ring-2 focus:ring-blue-900'
                                  : 'border-blue-300 bg-transparent focus:border-blue-400 focus:ring-2 focus:ring-blue-200'
                            } outline-none`}
                          />
                        </div>
                        {nameErrors[providerId] ? (
                    ...
                    {/* API Key input with label - expanded layout */}
                    <div className="space-y-1">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className={`w-full rounded-md border text-sm ${isDarkMode ? 'border-slate-600 bg-transparent text-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-800' : 'border-gray-300 bg-transparent text-gray-700 focus:border-blue-400 focus:ring-2 focus:ring-blue-200'} p-2 outline-none`}
                        />
                      ...
                      <div className="flex flex-col">
                        <div className="flex items-center">
                          ...
                          <input
                            ...
                            className={`flex-1 rounded-md border text-sm ${isDarkMode ? 'border-slate-600 bg-transparent text-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-800' : 'border-gray-300 bg-transparent text-gray-700 focus:border-blue-400 focus:ring-2 focus:ring-blue-200'} p-2 outline-none`}
                          />
                        </div>
                      </div>
                    )}
                    ...
                    {(providerConfig.type as ProviderTypeEnum) === ProviderTypeEnum.AzureOpenAI && (
                      <div className="flex items-start">
                        ...
                        <div className="flex-1 space-y-2">
                          <div
                            className={`flex min-h-[42px] flex-wrap items-center gap-2 rounded-md border ${isDarkMode ? 'border-slate-600 bg-transparent text-gray-200' : 'border-gray-300 bg-transparent text-gray-700'} p-2`}>
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          {(providerConfig.type as ProviderTypeEnum) === ProviderTypeEnum.OpenRouter ? (
                            <>
                              ...
                              <p className={`mt-1 mb-2 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    ...
                    {providerConfig.type === ProviderTypeEnum.Ollama && (
                      <div
                        className={`mt-4 rounded-md border ${isDarkMode ? 'border-slate-600 bg-transparent' : 'border-blue-100 bg-transparent'} p-3`}>
          ...
          <div className="provider-selector-container relative pt-2">
            <Button
              ...
              <span className="text-xs">{t('options_models_addNewProvider')}</span>
            </Button>

            {isProviderSelectorOpen && (
              <div
                className={`absolute z-10 mt-2 w-full overflow-hidden rounded-md border backdrop-blur-md animate-fade-in ${
                  isDarkMode
                    ? 'border-blue-500/30 bg-blue-900/20 shadow-lg shadow-slate-900/50'
                    : 'border-blue-200/50 bg-blue-50/80 shadow-xl shadow-blue-100/50'
                }`}>
                <div className="py-1">
                      ...
                      <button
                        ...
                        className={`flex w-full items-center px-4 py-3 text-left text-sm ${
                          isDarkMode
                            ...
                            : 'text-blue-700 hover:bg-blue-200/60 hover:text-blue-800'
      ...
      <div
        className={`rounded-lg border ${isDarkMode ? 'border-slate-700 bg-transparent' : 'border-blue-100 bg-transparent'} p-4 text-left shadow-sm`}>
        <h2 className={`mb-3 text-left text-lg font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
      ...
      <div
        className={`rounded-lg border ${isDarkMode ? 'border-slate-700 bg-transparent' : 'border-blue-100 bg-transparent'} p-4 text-left shadow-sm`}>
        <h2 className={`mb-2 text-left text-lg font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
