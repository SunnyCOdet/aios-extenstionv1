...
// Handle DevTools replay requests
chrome.runtime.onMessage.addListener((message, _sender, _sendResponse) => {
  if (message?.type === 'devtools_replay_request') {
    const req = message.request as { method: string; url: string; headers?: Record<string, string>; postData?: string };
    fetch(req.url, {
      method: req.method,
      headers: req.headers,
      body: ['GET', 'HEAD'].includes((req.method || 'GET').toUpperCase()) ? undefined : req.postData,
      // Use no-cors to avoid opaque failures for some hosts
      // mode: 'no-cors',
    }).catch(() => {
      // swallow errors; this is a best-effort replay
    });
  }
});
