...
export class PlannerAgent extends BaseAgent<typeof plannerOutputSchema, PlannerOutput> {
  ...
  async execute(): Promise<AgentOutput<PlannerOutput>> {
    try {
      // Check if we're in plan mode to show planning details
      const isPlanMode = this.context.options.mode === 'plan';
      this.context.emitEvent(Actors.PLANNER, ExecutionState.STEP_START, isPlanMode ? 'Planning...' : 'Thinking...');
      ...
      // If task is done, emit the final answer; otherwise show planning details based on mode
      if (cleanedPlan.done) {
        this.context.emitEvent(Actors.PLANNER, ExecutionState.STEP_OK, cleanedPlan.final_answer);
      } else {
        if (isPlanMode) {
          // In plan mode, show the next steps to user
          this.context.emitEvent(Actors.PLANNER, ExecutionState.STEP_OK, cleanedPlan.next_steps);
        } else {
          // In agent mode, don't show planning details to user
          this.context.emitEvent(Actors.PLANNER, ExecutionState.STEP_OK, '');
        }
      }
