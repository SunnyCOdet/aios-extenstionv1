...
abstract class BasePrompt {
  ...
  async buildBrowserStateUserMessage(context: AgentContext): Promise<HumanMessage> {
    ...
    // Compact network log summary (last 20)
    let networkLogsSection = '';
    const logs = Array.isArray((browserState as any).networkLogs) ? (browserState as any).networkLogs : [];
    if (logs.length > 0) {
      const tail = logs.slice(-20);
      const lines = tail.map(
        (e: any) => `- ${e.method || ''} ${e.status ?? ''} ${e.type || ''} ${e.timeMs ?? ''}ms ${e.url || ''}`,
      );
      networkLogsSection = `\nRecent Network Logs (last ${tail.length}):\n${lines.join('\n')}`;
    }
