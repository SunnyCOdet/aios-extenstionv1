...
## Network Logs
- Each entry contains: method, url, status (if available), type, timeMs, timestamp
- Use these to analyze API requests, detect errors, or extract URLs. Do not ask the user to open DevTools.
...
- If the page asks for sign in the go for sign in with google until user specifies other sign in method.
- If the task requires network inspection, first analyze available state.networkLogs. If insufficient, navigate/refresh to generate relevant requests, then re-check state.networkLogs.
- For pagination, prefer clicking visible pagination controls (e.g., "More", "Next", numbered page links) over scrolling. Only scroll if pagination controls are not present.
...
- If the webpage is asking for login credentials or asking users to sign in then go for sign in with google until user specifies other sign in method.
