...
export default class Page {
  ...
  private _networkLogs: NetworkLogEntry[] = [];
  ...
  async attachPuppeteer(): Promise<boolean> {
    ...
    // Attach lightweight network listeners for agent visibility
    this._attachNetworkLogListeners();

    return true;
  }

  private _attachNetworkLogListeners(): void {
    if (!this._puppeteerPage) return;

    const MAX_LOGS = 200;
    const entriesByRequest = new Map<string, NetworkLogEntry>();
    ...
    const onRequest = (request: HTTPRequest) => {
      try {
        ...
        // @ts-expect-error: _requestId is private but stable in practice
        const key = (request as any)._requestId || id;
        entriesByRequest.set(key, entry);
        this._networkLogs.push(entry);
        if (this._networkLogs.length > MAX_LOGS) this._networkLogs.shift();
      } catch (_) {}
    };
    ...
    const onResponse = (response: HTTPResponse) => {
      try {
        ...
        // @ts-expect-error: _requestId is private but stable in practice
        const key = (req as any)._requestId || '';
        const entry = entriesByRequest.get(key);
        if (entry) {
          entry.status = response.status();
          const now = Date.now();
          entry.timeMs = Math.max(0, now - entry.timestamp);
        }
      } catch (_) {}
    };

    // Keep references so listeners can be removed on detach
    (this as any)._nl_onRequest = onRequest;
    (this as any)._nl_onResponse = onResponse;
  ...
  async detachPuppeteer(): Promise<void> {
    if (this._browser) {
      // Remove network log listeners
      try {
        if (this._puppeteerPage && (this as any)._nl_onRequest && (this as any)._nl_onResponse) {
          this._puppeteerPage.off('request', (this as any)._nl_onRequest);
          this._puppeteerPage.off('response', (this as any)._nl_onResponse);
        }
      } catch (_) {}
  ...
  async _updateState(useVision = false, focusElement = -1): Promise<PageState> {
    ...
    try {
      ...
      // Use the displayHighlights setting directly, independent of useVision
      ...
      // include recent network logs (no bodies)
      this._state.networkLogs = this._networkLogs.slice(-100);
      return this._state;
    } catch (error) {
