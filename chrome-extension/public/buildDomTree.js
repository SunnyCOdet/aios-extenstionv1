window.buildDomTree = (
  ...
  function isInteractiveElement(element) {
    ...
    const interactiveElements = new Set([
      ...
      'video', // Media controls often clickable
      'audio',
    ]);
  ...
  function isHeuristicallyInteractive(element) {
    ...
    const hasInteractiveClass = /\b(btn|button|clickable|menu|item|entry|link|next|prev|more|pagination)\b/i.test(
      element.className || '',
    );
  ...
  function buildDomTree(node, parentIframe = null, isParentHighlighted = false, depth = 0) {
    ...
    if (
      ...
      // Enrich with computed hints for pagination/scroll
      try {
        const text = (node.innerText || node.textContent || '').toLowerCase().trim();
        if (text === 'next' || text === 'more' || /^(next|more|older|load more)$/i.test(text)) {
          nodeData.attributes['pagination'] = 'next';
        } else if (text === 'previous' || text === 'prev' || /^(previous|prev|newer)$/i.test(text)) {
          nodeData.attributes['pagination'] = 'prev';
        }

        // Detect numbered pagination containers
        if (node.tagName.toLowerCase() === 'nav' || node.className.match(/pagination|pager|pagenav|page-links/i)) {
          nodeData.attributes['pagination'] = nodeData.attributes['pagination'] || 'container';
        }

        // Detect scrollable containers (virtual list candidates)
        const style = getCachedComputedStyle(node);
        const overflowY = style?.overflowY || style?.overflow;
        const isScrollableY = (overflowY && /(auto|scroll)/.test(overflowY)) || node.scrollHeight > node.clientHeight;
        if (isScrollableY) {
          nodeData.attributes['scrollableY'] = 'true';
        }

        // Heuristic: virtual list if many similar children and container scrolls
        if (isScrollableY) {
          const children = Array.from(node.children || []);
          if (children.length >= 20) {
            const byTag = children.reduce((m, el) => {
              const t = el.tagName.toLowerCase();
              m[t] = (m[t] || 0) + 1;
              return m;
            }, {});
            const dominant = Object.values(byTag).some(c => c >= children.length * 0.6);
            if (dominant) {
              nodeData.attributes['virtualList'] = 'true';
            }
          }
        }
      } catch (e) {
